...
```bash
TEMPLATES_FILE="$SYMPHONY_ROOT/config/linear-templates.json"
cat "$TEMPLATES_FILE"
```
...
```bash
LINEAR_API_KEY="$(grep LINEAR_API_KEY $SYMPHONY_ROOT/secrets.env | cut -d= -f2)"
...
api_key = "$(grep LINEAR_API_KEY $SYMPHONY_ROOT/secrets.env | cut -d= -f2 | tr -d '\n')"
...
config = json.loads(open("$SYMPHONY_ROOT/config/linear-templates.json").read())
...
symphony_root = os.environ.get("SYMPHONY_ROOT", os.path.expanduser("~/symphony"))
api_key = open(os.path.join(symphony_root, "secrets.env")).read()
...
Templates are stored in `$SYMPHONY_ROOT/config/linear-templates.json`. The `descriptionMarkdown` field uses standard Markdown. Required sections per type:
