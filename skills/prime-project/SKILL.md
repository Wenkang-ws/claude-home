...
2. **Load project config** from `.claude/skills/prime-project/project-configs/{name}.json`
   - If config not found, tell the user and offer to create one
3. **Read each reference file** listed in the config's `references` array from `.claude/skills/prime-project/references/`
...
5. **Check for existing session state** in `.claude/skills/prime-project/session-state.md`
...
1. Create `.claude/skills/prime-project/project-configs/{name}.json` following the `payroll-backend.json` structure
2. Create `.claude/skills/prime-project/references/{name}/` with condensed convention files
